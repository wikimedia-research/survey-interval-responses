...
data {
  ...
  real prior_mu_1_mean;
  real prior_mu_1_sigma;
  
  real prior_mu_2_mean;
  real prior_mu_2_sigma;
...
transformed parameters {
  ...
  theta[1] = p*lognormal_cdf(edges[2], mu_1, sigma_1) + (1-p)*lognormal_cdf(edges[2], mu_1, sigma_1);
  for (i in 2:(n-1)) {
    theta[i] = p*lognormal_cdf(edges[i+1], mu_1, sigma_1) - p*lognormal_cdf(edges[i], mu_1, sigma_1);
    theta[i] += (1-p)*lognormal_cdf(edges[i+1], mu_2, sigma_2) - (1-p)*lognormal_cdf(edges[i], mu_2, sigma_2);
  }

  theta[n] = 1 - sum(theta[1:(n-1)]);
}
model {
    mu_1 ~ normal(prior_mu_1_mean, prior_mu_1_sigma);
    mu_2 ~ normal(prior_mu_2_mean, prior_mu_2_sigma);
