...
A natural extension of this methodology is regression and using predictors to model $\boldsymbol{\phi}$, the parameterization of $f$. In the first example, if more is known about the types of tasks being worked on or if there are multiple teams/departments taking the survey (or both!) those can be used in a regression model to assess the effect of the type of task on the median time to completion. In the second example, if more is know about the survey responders, that data can be used to assess differences in median income across demographics.
